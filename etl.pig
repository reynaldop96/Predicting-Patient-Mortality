...
eventswithmort= FOREACH eventswithmort GENERATE events::patientid as patientid, events::eventid as eventid, events::etimestamp as etimestamp, events::value as value, mortality::patientid, mortality::mtimestamp as mtimestamp, mortality::label as label;
-- perform join of events and mortality by patientid;

filterdead = FILTER eventswithmort BY (label ==1);
deadevents = FOREACH filterdead GENERATE mortality::patientid as patientid, eventid as eventid, value as value, label as label, DaysBetween(SubtractDuration(mtimestamp, 'P30D'), etimestamp) as time_difference; 
...
filtered = FILTER eventswithmort BY (label IS NULL);
grouped = GROUP filtered BY patientid;
index = FOREACH grouped GENERATE group AS patientid, MAX(filtered.etimestamp) as indexDate;
joined = JOIN filtered BY patientid, index BY patientid;
aliveevents = FOREACH joined GENERATE filtered::patientid as patientid, filtered::eventid as eventid, filtered::value as value, filtered::label as label, DaysBetween(indexDate, etimestamp) as time_difference;
...
-- ***************************************************************************
no_null= FILTER (UNION aliveevents, deadevents) BY value IS NOT NULL;
filtered = FILTER no_null by (time_difference<=2000L) AND (time_difference >= 0L);
...
-- ***************************************************************************
grouped = GROUP filtered BY (patientid, eventid);
featureswithid = FOREACH grouped GENERATE group.filtered::patientid as patientid, group.filtered::eventid as eventid, COUNT(filtered.filtered::eventid) as featurevalue;
...
-- ***************************************************************************
distinct_events = DISTINCT(FOREACH featureswithid GENERATE eventid);
ordered = ORDER distinct_events BY eventid;
ranked = RANK ordered;
all_features = FOREACH ranked GENERATE ($0-1) as idx, eventid;
...
joined= JOIN featureswithid BY eventid, all_features BY eventid;
features = FOREACH joined GENERATE patientid, idx, featurevalue;
...
-- ***************************************************************************
maxvalues = FOREACH (GROUP features BY all_features::idx) GENERATE group as idx, MAX(features.featurevalue) as maxvalue;
...
features = FOREACH normalized GENERATE patientid, all_features::idx as idx, (float) featurevalue/(float)maxvalue as normalizedfeaturevalue;
...
labels = DISTINCT(FOREACH filtered GENERATE patientid, (filtered::label IS NULL? 0:1));
