...
-- 1. sudo su - hdfs
-- 2. hdfs dfs -mkdir -p /input/sample_events
-- 3. hdfs dfs -chown -R root /input
-- 4. exit 
-- 5. hdfs dfs -put ../../data/sample_events.csv /input/sample_events/
...
-- ***** your code below *****
FROM events LEFT JOIN mortality on 
events.patient_id = mortality.patient_id
...
-- ***** your code below *****
FROM 
(SELECT COUNT(*) AS event_count FROM alive_events GROUP BY patient_id) as t1;
...
-- ***** your code below *****
FROM 
(SELECT COUNT(*) AS event_count FROM dead_events GROUP BY patient_id) as t2;
...
-- ***** your code below *****
FROM 
(SELECT COUNT(DISTINCT time) as encounter_count FROM alive_events GROUP BY patient_id) t1;
...
-- ***** your code below *****
FROM 
(SELECT COUNT(DISTINCT time) as encounter_count FROM dead_events GROUP BY patient_id) t1;
...
-- ***** your code below *****
FROM 
(SELECT datediff(max, min) as record_length FROM
(SELECT MAX(time) as max, MIN(time) as min from alive_events group by patient_id) t1) t2;
...
FROM 
(SELECT datediff(max, min) as record_length FROM
(SELECT MAX(time) as max, MIN(time) as min from dead_events group by patient_id) t1) t2;
...
GROUP BY event_id ORDER BY diag_count DESC LIMIT 5;
...
GROUP BY event_id ORDER BY lab_count DESC LIMIT 5;
...
GROUP BY event_id ORDER BY med_count DESC LIMIT 5;
